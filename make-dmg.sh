...
#   ./make-dmg.sh            -> build/Ask-for-Mac-<version>.dmg
...
DMG="build/Ask-for-Mac-$VERSION.dmg"
STAGE=$(mktemp -d)
...
Ask for Mac — how to install

1. Drag the "Ask for Mac" icon onto the "Applications" folder next to it.
...
4. Click the Apple menu (top-left) > System Settings > Privacy & Security.
   Scroll all the way down. Next to "Ask for Mac was blocked", click
...
5. Done. It opens like any other app from now on, and macOS never asks again.
   Ask a question about your files in your own words. The answer names the file it came from
   and can open it. Nothing leaves the Mac.
...
https://github.com/keithadler/askmac
TXT

rm -f "$DMG"
hdiutil create -volname "Ask for Mac" -srcfolder "$STAGE" -ov -format UDZO -quiet "$DMG"
